...
exports.renderPoweredByHtml = renderPoweredByHtml;
...
function renderPoweredByHtml(options = {}) {
    ...
    const anchor = `<a href="${escapeHtml(constants_js_1.PROSLOT_HREF)}" target="${constants_js_1.PROSLOT_TARGET}" rel="${constants_js_1.PROSLOT_REL}"` +
        (title ? ` title="${escapeHtml(title)}"` : '') +
        ` style="${linkStyle}">` +
        `${constants_js_1.PROSLOT_LABEL} <strong${nameStyle ? ` style="${nameStyle}"` : ''}>${constants_js_1.PROSLOT_NAME}</strong>` +
        `</a>`;
